...
export const FeaturedProducts: React.FC = () => {
  ...
  return (
    <section id="featured-products" className="relative py-14 sm:py-18 bg-slate-950 text-white border-b-2 border-slate-800 overflow-hidden">
      {/* Background Warehouse Overlay with Deep Dark Contrast */}
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
          <div>
            <div className="section-title text-orange-400">
              OUR FEATURED PRODUCTS
            </div>
            <h2 className="font-industrial text-2xl sm:text-3xl lg:text-4xl font-bold uppercase tracking-tight text-white">
              Explore Our Inventory
            </h2>
            <p className="text-slate-400 mt-1 text-sm max-w-2xl">
              Factory-grade replacement parts and heavy-duty assemblies ready for same-day dispatch across Central India and nationwide.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className={`px-3 py-1 text-xs font-mono font-bold uppercase rounded-[2px] border transition-colors ${
                activeFilter === 'all'
            ...
            <button
              type="button"
              onClick={() => setActiveFilter('tractor')}
              className={`px-3 py-1 text-xs font-mono font-bold uppercase rounded-[2px] border transition-colors ${
                activeFilter === 'tractor'
                  ? 'bg-orange-600 text-white border-orange-600'
                  : 'bg-slate-900 text-slate-300 border-slate-700 hover:bg-slate-800'
              }`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredProducts.map((product, idx) => (
            <div
              key={product.id}
              className="bg-slate-900/90 backdrop-blur-xs rounded-[4px] border border-slate-800 overflow-hidden flex flex-col justify-between hover:border-orange-500 transition-all duration-200 shadow-lg group"
            >
              <div>
                {/* Product Visual Mockup Image Banner */}
                <div className="relative h-48 bg-slate-950 border-b border-slate-800 overflow-hidden">
                  ...
                  <div className="absolute top-2.5 left-2.5 right-2.5 z-10 flex items-center justify-between">
                    <span className="bg-orange-600 text-white font-mono font-bold text-[10px] uppercase px-2 py-0.5 rounded-[2px] tracking-wider shadow-xs">
                      {product.badge}
                    </span>
                    <span className="text-[10px] font-mono text-slate-300 bg-slate-900/90 border border-slate-700 px-1.5 py-0.5 rounded-[2px]">
                      REF #{product.partNoSample}
                    </span>
                  </div>
                ...
                <div className="p-5 space-y-3">
                  <h3 className="font-industrial text-lg font-bold uppercase text-white tracking-wide group-hover:text-orange-400 transition-colors">
                    {product.title}
                  </h3>
                  ...
                  <div className="pt-2 border-t border-slate-800">
                    ...
                    <p className="text-slate-400 text-[11px] font-mono leading-tight">
              ...
              <div className="p-5 pt-0">
                <a
                  href={`https://wa.me/918707826228?text=Hello%20Patel%20Tractor%20Parts,%20I%20am%20inquiring%20about%20${encodeURIComponent(product.title)}%20(Ref:%20${product.partNoSample}).`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="accent-btn w-full text-xs py-2.5"
                >
        ...
        <div className="mt-8 bg-slate-900/90 backdrop-blur-xs p-4 rounded-[4px] border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-mono text-slate-400">
          ...
          <a
            ...
            className="text-orange-400 hover:text-orange-300 font-bold underline whitespace-nowrap"
          >
